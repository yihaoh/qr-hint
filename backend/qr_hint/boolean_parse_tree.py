...
class BNode:
    ...
    def __init__(self, val: str, type: str, select_xid: str = None, parent: "BNode" = None, is_distinct: bool = False):
        self.nid: str = str(uuid.uuid4())
        self.val: str = val
        self.type: str = type  # log, pred, expr, var, const
        self.children: "list[BNode]" = []
        self.z3_formula: str = None
        self.size: int = 0
        self.bounds: tuple[str, str] = None
        self.parent: "BNode" = parent
        self.select_xid: str = select_xid
        self.is_distinct: bool = is_distinct  # For aggregate functions: COUNT(DISTINCT col), etc.
        ...
        if self.children:
            if self.type == "log":
                ...
                if self.val in ["+", "-", "*", "/"]:
                    ...
                    self.z3_formula = f"{self.children[0].getZ3()} + {self.children[1].getZ3()}"
                elif self.val in ["AVG", "COUNT", "MAX", "MIN", "SUM", "COUNTINT", "COUNTSTR"]:
            ...
            self.z3_formula = (
                self.val if self.val.replace(".", "").replace("-", "").isnumeric() else f"StringVal({self.val})"
            )
