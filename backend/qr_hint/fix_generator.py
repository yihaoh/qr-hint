...
from .query_info import COUNTINT, COUNTSTR, MIN, MAX, AVG, SUM, DISTINCT
...
class FixGenerator:
    ...
    def derive_fixes(self, root: BNode, lower: BoolRef, upper: BoolRef, rs: list):
        ...
        """
        if root.bounds[0] == root.bounds[1]:
            return []
        elif root.val == "Not":
        ...
        for i in range(len(nrs_cur_level)):
            ...
            for n in residual:
                tp_repair_lower = (
                    And(tp_repair_lower, eval(n.bounds[0]))
                    if root.val == "And"
                    else Or(tp_repair_lower, eval(n.bounds[0]))
                )
                tp_repair_upper = (
                    And(tp_repair_upper, eval(n.bounds[1]))
                    if root.val == "And"
                    else Or(tp_repair_upper, eval(n.bounds[1]))
                )

            tp_target_lower = (
                lower if root.val == "And" else Or(And(lower, Not(tp_repair_lower)), eval(nrs_cur_level[i].bounds[0]))
            )
            tp_target_upper = (
                upper if root.val == "Or" else And(Or(upper, Not(tp_repair_upper)), eval(nrs_cur_level[i].bounds[1]))
            )
    ...
    def minimize_target(self, lower: BoolRef, upper: BoolRef):
        ...
        """
        tt = pd.DataFrame(
            columns=self.all_preds + ["lower", "upper", "final"], index=range(0, 2 ** (len(self.all_preds)))
        )
        ...
        while index < end:
            ...
            if self.solver.check() == sat:
                ...
                tt.at[index, "final"] = tt.at[index, "upper"] if tt.at[index, "lower"] == tt.at[index, "upper"] else "-"

                truth_val += tt.at[index, "final"]
                if tt.at[index, "final"] == "1":
                    minterms.append(index)
                elif tt.at[index, "final"] == "-":
        ...
        # Espresso
        X = ttvars("x", len(alias))
        f1 = truthtable(X, truth_val)
        (f1m,) = espresso_tts(f1)
        f1m = f1m.to_dnf()
        sols = ast_to_str(f1m.to_ast(), len(alias))
        ...
        for term in terms:
            ...
            while j < len(term):
                ...
                if j + 1 < len(term) and term[j + 1] == "'":
                    ...
                    cur_clause = (
                        And(cur_clause, Not(self.all_preds_z3[k]))
                        if cur_clause is not None
                        else Not(self.all_preds_z3[k])
                    )
                    j += 2
                else:
                    sz += 2 if cur_clause is not None else 1
                    cur_clause = (
                        And(cur_clause, self.all_preds_z3[k]) if cur_clause is not None else self.all_preds_z3[k]
                    )
                    j += 1
